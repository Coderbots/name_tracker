...
userRoutes.route('/').post((req,res) => {
	...
	User.findOneAndUpdate({person_name:name}, {$inc : {'freq_count' : 1}, $set: {'req_time' : new Date()}})
		.then(user => {
...
userRoutes.route('/topfive').get(function (req,res){
	User.find().sort({'freq_count': -1}).limit(5).select('person_name freq_count req_time -_id')
	.then(users => {
		res.json(users);
	})
	.catch(err => {
...
userRoutes.route('/mostrecent').get(function (req,res){
	...
	User.find().sort({'req_time': -1}).limit(1).select('person_name req_time -_id')
	.then(users => {
		res.json(users);
